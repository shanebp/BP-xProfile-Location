...
if ( !class_exists('PP_Field_Location') ) {
	...
	class PP_Field_Location {
		...
		function __construct () {
            ...
            add_filter( 'bps_field_validation_type',        array($this, 'pp_loc_profile_search_field' ) );
            add_filter( 'bps_field_type_for_search_form',   array($this, 'pp_loc_profile_search_field' ) );
            add_filter( 'bps_field_type_for_query',         array($this, 'pp_loc_profile_search_field' ) );
		}
		...
		function pp_loc_xprofile_data_after_save( $data ) {
			...
			if ( $field->type == 'location' ) {
				...
				if( ! empty( $_POST['pp_'.$data->field_id.'_geocode'] ) ) {
		...
		/* filters re BP Profile Search */

		public function pp_loc_profile_search_field( $field_type ) {
